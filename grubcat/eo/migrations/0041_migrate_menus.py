...
from eo.models import Dish, DishItem, DishCategoryItem
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for menu in orm.Menu.objects.all():
            for item in menu.items.all():
                object_type = ContentType.objects.get(id=item.content_type_id)
                object = object_type.get_object_for_this_type(id=item.object_id)
                if  isinstance(object, Dish):
                    dish_item = DishItem(menu_id=menu.id,dish=object,num=item.num,order_no=item.id+1)
                    dish_item.save()
                else:
                    dish_category_item = DishCategoryItem(menu_id=menu.id,category=object,order_no=item.id+1)
                    dish_category_item.save()
            menu.save()
    ...
    models = {
        ...
        'eo.meal': {
            ...
            'restaurant': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['eo.Restaurant']", 'null': 'True', 'blank': 'True'}),
            'start_date': ('django.db.models.fields.DateField', [], {'default': 'datetime.datetime(2012, 8, 17, 0, 0)'}),
        ...
        'eo.mealcomment': {
            ...
            'timestamp': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 8, 17, 0, 0)'})
        },
        'eo.mealinvitation': {
