...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for dish in orm.Dish.objects.all():
            for category in dish.categories.exclude(restaurant=dish.restaurant):
                dish.categories.remove(category)
        for categoryItem in orm.DishCategoryItem.objects.all():
            try:
                if categoryItem.category.restaurant != categoryItem.menu.restaurant:
                    new_category = orm.DishCategory.objects.get_or_create(name=categoryItem.category.name, restaurant=categoryItem.menu.restaurant)[0]
                    categoryItem.category = new_category
                    categoryItem.save()

            except ObjectDoesNotExist:
                print categoryItem.category.id
                print categoryItem.category.name
                new_category = orm.DishCategory.objects.get_or_create(name=categoryItem.category.name, restaurant=categoryItem.menu.restaurant)[0]
                categoryItem.category = new_category
                categoryItem.save()
