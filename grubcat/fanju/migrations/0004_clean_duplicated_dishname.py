...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for rest in orm.Restaurant.objects.all():
            duplications = rest.dish_set.values('restaurant', 'name').annotate(count = Count('name')).filter(count__gt=1)

            if len(duplications):
                for entry in duplications:
                    logger.info( 'find duplicated name %s in restaurant %s' % (entry['name'], entry['restaurant']))
                    for idx, dish in enumerate(orm.Dish.objects.filter(restaurant=entry['restaurant'], name=entry['name'])):
                        dish.name= dish.name + 'dup' + str(idx)
                        dish.save()
    ...
    models = {
        ...
        u'fanju.meal': {
            ...
            'start_date': ('django.db.models.fields.DateField', [], {'default': 'datetime.datetime(2013, 5, 20, 0, 0)'}),
        ...
        u'fanju.menu': {
            ...
            'created_time': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 5, 20, 0, 0)'}),
